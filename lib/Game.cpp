...
void Game::Render()
{
	Engine::getInstance().GetRenderer()->StartFrame();
	...
	case Game::PLAY_MODE:
		{
			//Debug
			{
				...
				time << m_dCollisionTime;
				m_pFont->RenderText(time.str(), x, y, w, 0.0f, w, h, r);
			}
	...
	Engine::getInstance().GetRenderer()->EndFrame();
}
...
Actor CreateNewAsteroid(const Actor& pCopy, int size)
{
	...
	temp.m_size = g_asteroidRadii[size];
	temp.m_radius = g_asteroidRadii[size];
	temp.m_actorType = enumObjectsId::Asteroid;
	temp.m_alive = true;
...
void RandomiseAsteroid(Actor& pActor, int size)
{
	...
	pActor.m_size = g_asteroidRadii[size];
	pActor.m_radius = g_asteroidRadii[size];
	pActor.m_actorType = enumObjectsId::Asteroid;
	pActor.m_alive = true;
}
...
void ResetPlayerPos()
{
	...
	if (pSceneNodeShip != nullptr)
	{
		...
		actor.m_radius = 20;
		actor.m_alive = true;
		actor.m_actorType = enumObjectsId::Ship;
...
void Game::CheckCollision()
{
	...
	//Start timer
	Engine::getInstance().StartTimer();

	std::vector<Actor>& bulletActors = pBulletNode->GetActors();
	for (auto& bulletActor : bulletActors)
	{
		std::vector<Actor*> bounds;
		if (bulletActor.m_alive)
		{
			Bounds shipBounds(bulletActor.m_x, bulletActor.m_y, bulletActor.m_size, bulletActor.m_size);
			Engine::getInstance().GetSceneManager()->GetQuadTree()->retrieve(bounds, &bulletActor);
 			for (auto& asteroid : bounds)
			{
				if (asteroid->m_actorType == enumObjectsId::Asteroid)
				{
					if (pBulletNode->CheckCollision(bulletActor, *asteroid))
					{
	...
	*/
	...
	//Get last time
	m_dCollisionTime = Engine::getInstance().EndTimer();
}
