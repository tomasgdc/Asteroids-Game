...
void BulletNode::VRender()
{
	for (auto& actor : m_itActors)
	{
		if (actor.m_alive)
		{
			...
			Engine::getInstance().GetRenderer()->DrawRectangular(actor.m_x, actor.m_y, actor.m_radius, actor.m_radius);
		}
	}
}
...
void BulletNode::OnFireEvent()
{
	...
	*/
 	...
 	if (IsKeyDown(VK_SPACE))
	{
		...
		static float bulletVelocity = 8.0f;
		...
		m_itActors[m_nextFiredBulletIndex].m_size = 8;
		m_itActors[m_nextFiredBulletIndex].m_alive = true;
		m_itActors[m_nextFiredBulletIndex].m_actorType = enumObjectsId::Bullet;
...
bool BulletNode::CheckCollision(Actor & bullet, const Actor & pActor)
{
	if (SphereCollision(bullet, pActor))
	{
		bullet.m_alive = false;
		return true;
	}
	return false;
}
